        ...
        return (
            <div className="mt-4 pt-4 border-t border-white/10">
                ...
                <div className="flex flex-wrap gap-3 mt-3">
                    ...
                    <button onClick={() => setDeclining(false)} disabled={saving} className="px-4 py-2.5 rounded-lg text-sm font-semibold text-gray-300 hover:text-white border border-white/10">Back</button>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="mt-4 pt-4 border-t border-white/10 grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="sm:col-span-2 flex flex-wrap gap-3 mt-1">
                ...
                <button onClick={() => setDeclining(true)} disabled={saving} className="px-4 py-2.5 rounded-lg text-sm font-semibold text-gray-300 hover:text-white border border-white/10">Can't assist</button>
            </div>
        </div>
    );
};
            ...
            <div className="flex justify-between items-start gap-3">
                ...
                <div className="text-right shrink-0">
                    {rfq.status === 'Open' && rfq.closesAt && (
                        <span className={`block text-xs font-black px-2.5 py-1 rounded-lg ${cd.over ? 'bg-gray-700 text-gray-400' : cd.urgent ? 'bg-red-500/20 text-red-300' : 'bg-emerald-500/15 text-emerald-300'}`}>
                            {cd.over ? 'closed' : `⏳ ${cd.text}`}
                        </span>
                    )}
                    <span className="block text-[10px] text-gray-600 whitespace-nowrap mt-1">{format(new Date(rfq.createdAt), 'dd MMM')}</span>
                </div>
            </div>
...
const SupplierRfqList: React.FC<{ supplier: Supplier }> = ({ supplier }) => {
    ...
    return (
        <div>
            ...
            <p className="text-gray-400 mb-6">Loads we'd like your rate on. Submit a price if you can assist, before the deadline.</p>
