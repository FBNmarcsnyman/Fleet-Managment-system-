    ...
    const handlers = useMemo(() => ({
        ...
        handleCreateManifest: async (payload: any) => {
            try {
                ...
                const row: any = { organization_id: FBN_ORGANIZATION_ID, manifest_number: number, origin_branch_id: branchIdByName.get(payload.originBranch) || null, destination_branch_id: branchIdByName.get(dest) || null, dispatch_date: today, vehicle_id: payload.vehicleId || null, driver_id: manDriver, load_confirmation_ids: ids, status: 'In Transit', trailer_size: payload.trailerSize || null, total_rate: payload.totalRate ?? null, start_odometer: payload.startOdometer ?? null, trailer_reg_6m: payload.trailerReg6m || null, trailer_reg_12m: payload.trailerReg12m || null, trailer_split: payload.trailerSplit || null, carrier_name: payload.carrierName || null, carrier_vehicle_reg: payload.carrierVehicleReg || null, carrier_driver: payload.carrierDriver || null, carrier_cell: payload.carrierCell || null, carrier_email: payload.carrierEmail || null };
