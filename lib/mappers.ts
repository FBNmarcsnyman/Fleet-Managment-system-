...
export const mapFuelEntry = (row: Tables['fuel_entries']['Row']): FuelEntry => ({
    ...
    date: row.date,
    time: (row as any).fill_time ?? undefined,
...
export const mapLoadConfirmation = (row: Tables['load_confirmations']['Row'], ctx: MapperCtx): LoadConfirmation => ({
    ...
    storedSince: (row as any).stored_since ?? undefined,
    freeDays: (row as any).free_days ?? undefined,
    handlingCharges: Array.isArray((row as any).handling_charges) ? (row as any).handling_charges : undefined,
    totalAmount: row.total_amount,
    ...
    podAuthorisation: (row as any).pod_authorisation ?? undefined,
    podOriginalUrl: (row as any).pod_original_url ?? undefined,
    ...
    driverAcceptedAt: (row as any).driver_accepted_at ?? undefined,
    collectionArrivedAt: (row as any).collection_arrived_at ?? undefined,
    collectionDepartedAt: (row as any).collection_departed_at ?? undefined,
...
export const toFuelEntryInsert = (
    ...
    date: entry.date,
    fill_time: (entry as any).time ?? null,
...
export const toLoadConfirmationUpdate = (
    ...
    if ((updates as any).storedSince !== undefined) (row as any).stored_since = (updates as any).storedSince ?? null;
    if ((updates as any).freeDays !== undefined) (row as any).free_days = (updates as any).freeDays ?? null;
    if ((updates as any).handlingCharges !== undefined) (row as any).handling_charges = (updates as any).handlingCharges ?? null;
    if (updates.totalAmount !== undefined) row.total_amount = updates.totalAmount;
...
export const toRfqRequestInsert = (rfq: Partial<RfqRequest>, requestNumber: string, createdBy?: string) => ({
    ...
    packages: (rfq as any).packages ?? null,
    dimensions: (rfq as any).dimensions ?? null,
    cube_m3: (rfq as any).cubeM3 ?? null,
    git_required: rfq.gitRequired ?? false,
    hazardous: rfq.hazardous ?? false,
